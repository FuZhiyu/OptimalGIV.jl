...
@testset "test get_coefnames" begin
    ...
    response_name, endog_name, elasticity_names, covariates_names, slope_terms = get_coefnames(df_coef, formula1)
    @test response_name == "q"
    @test endog_name == "p"
    @test elasticity_names == ["id: 1 & p", "id: 2 & p", "id: 3 & p"]
    @test covariates_names == ["(Intercept)", "η"]
    # Test 2: Formula with continuous variable interaction
    formula2 = @formula(q + C1 & endog(p) ~ η)
    response_name, endog_name, elasticity_names, covariates_names, slope_terms = get_coefnames(df_coef, formula2)
    @test response_name == "q"
    @test endog_name == "p"
    @test elasticity_names == ["C1 & p"]
    @test covariates_names == ["(Intercept)", "η"]

    # Test 3: Formula with multiple interactions
    formula3 = @formula(q + id & endog(p) + C1 & endog(p) ~ η + id)
    response_name, endog_name, elasticity_names, covariates_names, slope_terms = get_coefnames(df_coef, formula3)
    @test response_name == "q"
    @test endog_name == "p"
    @test elasticity_names == ["id: 1 & p", "id: 2 & p", "id: 3 & p", "C1 & p"]
    @test covariates_names == ["(Intercept)", "η", "id: 2", "id: 3"]

    # Test 4: Formula with intercept
    formula4 = @formula(q + endog(p) ~ 0 + id)
    response_name, endog_name, elasticity_names, covariates_names, slope_terms = get_coefnames(df_coef, formula4)
    @test response_name == "q"
    @test endog_name == "p"
    @test elasticity_names == ["p"]
    ...
    response_name, endog_name, elasticity_names, covariates_names, slope_terms = get_coefnames(df_coef, formula5)
    @test response_name == "q"
    @test endog_name == "p"
    @test elasticity_names == ["C2: A & p", "C2: B & p", "C2: C & p"]
    @test covariates_names == ["η"]

    # Test 6: Formula with no interactions (constant elasticity)
    formula6 = @formula(q + endog(p) ~ η + 0)
    response_name, endog_name, elasticity_names, covariates_names, slope_terms = get_coefnames(df_coef, formula6)
    @test response_name == "q"
    @test endog_name == "p"
    @test elasticity_names == ["p"]
    @test covariates_names == ["η"]
    ...
    response_name, endog_name, elasticity_names, covariates_names, slope_terms = get_coefnames(df_coef, formula7)
    @test covariates_names == ["η"]
end
...
@testset "test parse_guess" begin
    ...
    elasticity_names_single = ["p"]
    elasticity_names_multi = ["id: 1 & p", "id: 2 & p", "C1 & p"]

    # Test 1: Vector of numbers - should return as is
    guess_vec = [1.0, 2.0, 3.0]
    @test parse_guess(elasticity_names_multi, guess_vec, Val(:iv)) == guess_vec
    @test parse_guess(elasticity_names_multi, guess_vec, Val(:debiased_ols)) == guess_vec

    # Test 2: Nothing - should return nothing
    @test isnothing(parse_guess(elasticity_names_single, nothing, Val(:iv)))
    @test isnothing(parse_guess(elasticity_names_multi, nothing, Val(:debiased_ols)))

    # Test 3: Single number - should wrap in vector
    @test parse_guess(elasticity_names_single, 2.5, Val(:iv)) == [2.5]
    @test parse_guess(elasticity_names_single, -1.2, Val(:debiased_ols)) == [-1.2]

    # Test 4: Dict for scalar_search algorithm - needs "Aggregate" key
    guess_dict_scalar = Dict("Aggregate" => 0.8)
    @test parse_guess(elasticity_names_single, guess_dict_scalar, Val(:scalar_search)) == [0.8]

    guess_dict_scalar_vec = Dict("Aggregate" => [0.8])
    @test parse_guess(elasticity_names_single, guess_dict_scalar_vec, Val(:scalar_search)) == [0.8]

    # Test 5: Dict for scalar_search without "Aggregate" key - should throw error
    guess_dict_wrong = Dict("id: 1 & p" => 1.0)
    @test_throws ArgumentError parse_guess(elasticity_names_single, guess_dict_wrong, Val(:scalar_search))
    ...
    result = parse_guess(elasticity_names_multi, guess_dict, Val(:iv))
    ...
    @test_throws ArgumentError parse_guess(elasticity_names_multi, guess_dict_incomplete, Val(:iv))
    ...
    result_symbols = parse_guess(elasticity_names_multi, guess_dict_symbols, Val(:debiased_ols))
    ...
    result_extra = parse_guess(elasticity_names_multi, guess_dict_extra, Val(:iv))
    @test result_extra == [1.0, 2.0, 0.5]

    # Test 10: Single elasticity with dict
    guess_dict_single = Dict("p" => 0.7)
    @test parse_guess(elasticity_names_single, guess_dict_single, Val(:iv)) == [0.7]

    # Test 11: Empty elasticity names (edge case)
    elasticity_names_empty = String[]
    @test parse_guess(elasticity_names_empty, Float64[], Val(:iv)) == Float64[]
    @test isnothing(parse_guess(elasticity_names_empty, nothing, Val(:iv)))
    @test parse_guess(elasticity_names_empty, Dict{String,Float64}(), Val(:iv)) == Float64[]
end
