...
@testset "homogeneous elasticity" begin
    ...
    @test endog_coef(givmodel)[1] * 2 ≈ 2.5341730 atol = 1e-4
    @test stderror(givmodel)[1] * 2 ≈ 0.2407 atol = 1e-4
    factor_coef = [0.2419 1.1729; 0.1842 0.3722; -1.3213 -0.6487; 0.6288 -0.4422; 0.7269 1.6341]
    @test maximum(abs.(Matrix(givmodel.fe[:, 2:3]) - factor_coef)) < 1e-4
    ...
    factor_coef = [0.2419 1.1729; 0.1842 0.3722; -1.3213 -0.6487; 0.6288 -0.4422; 0.7269 1.6341]
    @test exog_coef(givmodel_partialabsorbed) ≈ factor_coef[:, 1] atol = 1e-4
...
@testset "heterogeneous elasticity" begin
    ...
    @test endog_coef(givmodel) ≈ [1.59636, 1.657, 1.29643, 3.33497, 0.58443] atol = 1e-4
    # println(round.(sqrt.(diag(est.Σζ)), digits = 4))
    givse = stderror(givmodel)[1:5]
    ...
    @test exog_coef(givmodel) ≈ factor_coef atol = 1e-4
